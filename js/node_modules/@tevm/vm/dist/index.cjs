...
var _runTx = (vm) => async (opts) => {
  ...
  let blobVersionedHashes = void 0;
  if (tx$1 instanceof tx.BlobEIP4844Transaction) {
    blobVersionedHashes = tx$1.blobVersionedHashes;
  }
...
var applyTransactions = (vm$1) => async (block, opts) => {
  ...
  for (let txIdx = 0; txIdx < block.transactions.length; txIdx++) {
    ...
    if (vm$1.common.ethjsCommon.isActivatedEIP(1559) === true) {
      maxGasLimit = block.header.gasLimit * vm$1.common.ethjsCommon.param("elasticityMultiplier");
    } else {
...
var BlockBuilder = class {
  ...
  constructor(vm, opts) {
    ...
    this.headerData = {
      ...
      timestamp: opts.headerData?.timestamp ?? Math.round(Date.now() / 1e3)
    };
