...
var InvalidJsonError = class extends errors.BaseError {
  /**
...
var ReadRequestBodyError = class extends errors.BaseError {
  /**
...
var handleBulkRequest = async (client, requests) => {
  const { send } = client.transport.tevm.extend(decorators.tevmSend());
  ...
  return responses.map((response, i) => {
    ...
    if (response.status === "rejected") {
      ...
      const err = new errors.InternalError(request.method, { cause: response.reason });
      return {
...
var zJsonRpcRequest = zod.z.object({
  jsonrpc: zod.z.literal("2.0").optional(),
  id: zod.z.union([zod.z.string(), zod.z.number(), zod.z.null()]).optional(),
  method: zod.z.string(),
  params: zod.z.unknown().optional()
});
var zBulkRequest = zod.z.array(zJsonRpcRequest);
var parseRequest = (body) => {
  ...
  if (!parsedRequest.success) {
    return new errors.InvalidRequestError(JSON.stringify(parsedRequest.error.format()));
  }
...
var createHttpHandler = (client) => {
  return async (req, res) => {
    ...
    if (parsedRequest instanceof InvalidJsonError || parsedRequest instanceof errors.InvalidRequestError) {
    ...
    const response = await client.transport.tevm.extend(decorators.tevmSend()).send(
      /** @type any*/
      parsedRequest
    ).catch((e) => {
      console.log("e", e);
      return "code" in e ? e : new errors.InternalError("Unexpeced error", { cause: e });
    });
    if ("code" in response && "message" in response) {
      return handleError(client, response, res, parsedRequest);
    }
    if (((_a = response.error) == null ? void 0 : _a.code) === errors.UnsupportedProviderMethodError.code || ((_b = response.error) == null ? void 0 : _b.code) === errors.MethodNotFoundError.code) {
...
var createServer = (client, serverOptions = {}) => {
  return http.createServer(serverOptions, createHttpHandler(client));
};
...
exports.ReadRequestBodyError = ReadRequestBodyError;
