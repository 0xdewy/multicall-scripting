...
export const options = zod.object({
    force: zod.boolean().default(false).describe(option({
        description: 'Overwrite existing files',
        alias: 'f',
    })),
    dir: zod.string().default(process.cwd()).describe(option({
        description: 'Working directory (defaults to current directory)',
        alias: 'd',
    })),
    include: zod.string().default('src/**/*.sol').describe(option({
        description: 'Glob pattern(s) for Solidity files, comma-separated',
        alias: 'i',
    })),
    output: zod.string().optional().describe(option({
        ...
        return (React.createElement(Box, { flexDirection: "column" },
            React.createElement(Box, null,
                ...
                React.createElement(Text, null,
                    ...
                    name ? `"${name}"` : "files",
                    options.force ? ' (force)' : '')),
            currentTask && (React.createElement(Box, { marginTop: 1 },
                React.createElement(Text, { dimColor: true }, currentTask)))));
    }
    return (React.createElement(Box, { flexDirection: "column" },
        result.success ? (React.createElement(React.Fragment, null,
            React.createElement(Text, { color: "green" },
                "Successfully generated ",
                result.files.length,
                " files:"),
            React.createElement(Newline, null),
            result.files.map((file, index) => (React.createElement(Text, { key: index },
                "- ",
                file))))) : (React.createElement(React.Fragment, null,
            React.createElement(Text, { color: "red" }, "Generation failed with errors:"),
            React.createElement(Newline, null),
            result.errors.map((error, index) => (React.createElement(Text, { key: index, color: "red" },
                "- ",
                error))))),
        result.files.length > 0 && result.errors.length > 0 && (React.createElement(React.Fragment, null,
            React.createElement(Newline, null),
            React.createElement(Text, { color: "yellow" }, "Completed with warnings:"),
            React.createElement(Newline, null),
            result.errors.map((error, index) => (React.createElement(Text, { key: index, color: "yellow" },
                "- ",
                error)))))));
}
