...
export const options = z.object({
    ...
    args: z.string().optional().describe(option({
        description: optionDescriptions.args,
    })),
    // Basic call parameters
    from: z.string().default(envVar('from') || '0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266').describe(option({
        description: optionDescriptions.from,
        defaultValueDescription: '0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266',
    })),
    value: z.string().optional().describe(option({
        description: optionDescriptions.value,
    })),
    // Contract code options
    deployedBytecode: z.string().optional().describe(option({
        description: optionDescriptions.deployedBytecode,
    })),
    code: z.string().optional().describe(option({
        description: optionDescriptions.code,
    })),
    // Gas parameters
    gas: z.string().optional().describe(option({
        description: optionDescriptions.gas,
    })),
    gasPrice: z.string().optional().describe(option({
        description: optionDescriptions.gasPrice,
    })),
    maxFeePerGas: z.string().optional().describe(option({
        description: optionDescriptions.maxFeePerGas,
    })),
    maxPriorityFeePerGas: z.string().optional().describe(option({
        description: optionDescriptions.maxPriorityFeePerGas,
    })),
    gasRefund: z.string().optional().describe(option({
        description: optionDescriptions.gasRefund,
    })),
    // Block options
    blockTag: z.string().optional().describe(option({
        description: optionDescriptions.blockTag,
    })),
    // Advanced options
    caller: z.string().optional().describe(option({
        description: optionDescriptions.caller,
    })),
    origin: z.string().optional().describe(option({
        description: optionDescriptions.origin,
    })),
    depth: z.number().optional().describe(option({
        description: optionDescriptions.depth,
    })),
    skipBalance: z.boolean().optional().describe(option({
        description: optionDescriptions.skipBalance,
    })),
    // Instrumentation options
    createTrace: z.boolean().default(false).describe(option({
        description: optionDescriptions.createTrace,
    })),
    createAccessList: z.boolean().default(false).describe(option({
        description: optionDescriptions.createAccessList,
    })),
    createTransaction: z.enum(['on-success', 'always', 'never']).default('never').describe(option({
        description: optionDescriptions.createTransaction,
    })),
    // Output formatting
    formatJson: z.boolean().default(envVar('format_json') !== 'false').describe(option({
...
export default function Contract({ options }) {
    ...
    return (React.createElement(CliAction, { ...actionResult, targetName: `contract ${actionResult.options['to'] || 'code'} function ${actionResult.options['functionName']}`, successMessage: "Contract function called successfully!" }));
}
