...
var eventsFactory = ({ abi, bytecode, deployedBytecode, address }) => Object.fromEntries(
  abi.filter((field) => {
    ...
    const creator = (params) => {
      return {
        ...
        abi: [eventAbi],
        humanReadableAbi: utils.formatAbi([eventAbi]),
        bytecode,
        deployedBytecode,
        address,
        ...params
    ...
    eventAbi.name;
    creator.humanReadableAbi = utils.formatAbi([eventAbi]);
...
var readFactory = ({ methods, address, code }) => Object.fromEntries(
  methods.filter(({ type }) => type === "function").filter((method) => {
    ...
    const creator = (...args) => {
      ...
      return {
        abi: methodAbi,
        humanReadableAbi: utils.formatAbi([method]),
        functionName: (
          /**@type {import('@tevm/utils').AbiFunction}*/
          method.name
        ),
    ...
    creator.abi = [method];
    creator.humanReadableAbi = utils.formatAbi([method]);
    creator.code = code;
    creator.address = address;
    creator.to = address;
    return [
...
var writeFactory = ({ methods, address, code }) => Object.fromEntries(
  methods.filter(({ type }) => type === "function").filter((method) => {
    const abiFunction = (
      /** @type {import('@tevm/utils').AbiFunction} */
      method
...
var createContract = ({
  ...
  const { abi, humanReadableAbi } = (() => {
    if (_abi) {
      return { abi: _abi, humanReadableAbi: utils.formatAbi(_abi) };
    }
    if (_humanReadableAbi) {
      return { abi: utils.parseAbi(_humanReadableAbi), humanReadableAbi: _humanReadableAbi };
    }
    throw new errors.InvalidParamsError("Must provide either humanReadableAbi or abi", {
  ...
  const optionalArgs = {
    ...address !== void 0 ? { address: utils.getAddress(address) } : {},
    ...code !== void 0 ? { code } : {},
  ...
  const withAddress = (address2) => {
    const formattedAddress = utils.getAddress(address2);
