  ...
  if (result.errors && result.errors.length > 0) {
    result.errors = result.errors.map((err) => {
      if (utils.isHex(err.message) && err instanceof errors.RevertError) {
        ...
        try {
          decodedError = utils.decodeErrorResult(
            /** @type {any} */
            {
              abi: params.abi,
              data: err.message,
          ...
          return new errors.RevertError(message, { cause: err });
        } catch (e) {
  ...
  try {
    ...
    if ("dumpCanonicalGenesis" in vm.stateManager) {
      ...
      if (await vm.stateManager.hasStateRoot(block.header.stateRoot)) {
        return { state: vm.stateManager._baseState.stateRoots.get(utils.bytesToHex(block.header.stateRoot)) ?? {} };
      }
...
var AccountStorage = zBaseParams.extend({
  nonce: zod.z.bigint().describe("The nonce of the account"),
  balance: zod.z.bigint().describe("The balance of the account"),
  storageRoot: zHex.describe("The storage root of the account"),
  codeHash: zHex.describe("The code hash of the account"),
  storage: zod.z.optional(zod.z.record(zHex)).describe("The storage of the account")
});
var zLoadStateParams = zod.z.object({ state: zod.z.record(AccountStorage) }).describe("Properties shared across the load state");
...
var anvilResetJsonRpcProcedure = (node) => {
  return async (request) => {
    ...
    txs.forEach((tx) => txPool.removeByHash(viem.bytesToHex(tx.hash())));
...
var getBalanceHandler = (baseClient) => async ({ address: address$1, blockTag = "latest" }) => {
  ...
  const block = vm.blockchain.blocks.get(
    /** @type any*/
    typeof blockTag === "string" && blockTag.startsWith("0x") ? utils.hexToBytes(
      /** @type {import('@tevm/utils').Hex}*/
      blockTag
    ) : blockTag
  ...
  if (hasStateRoot) {
    ...
    return root?.[address$1]?.balance ?? 0n;
  }
