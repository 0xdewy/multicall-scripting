...
fn scalar_chaining_offsets_match_codec() {
    ...
    // x = add(2, 2)
    let x = b.add_call(add, target, vec![uint(2), uint(2)], U256::ZERO).unwrap();
    // y = add(2, x)  — x is spliced into arg index 1 (memTarget 4 + 32*1 = 36)
    let y = b
        .add_call(add, target, vec![uint(2), Arg::Ref(x[0].clone())], U256::ZERO)
        .unwrap();
    ...
    assert_eq!(out.calldatas[0].len(), 4 + 64);
    assert_eq!(out.calldatas[2].len(), 4 + 32);
}

#[test]
fn rejects_descriptor_reuse() {
    let abi: JsonAbi = serde_json::from_str(ABI).unwrap();
    let add = &abi.function("add").unwrap()[0];
    let target = Address::ZERO;

    let mut b = TransactionBuilder::new();
    let x = b.add_call(add, target, vec![uint(1), uint(2)], U256::ZERO).unwrap();
    b.add_call(add, target, vec![uint(3), Arg::Ref(x[0].clone())], U256::ZERO)
        .unwrap();
    // reusing the same return value must fail
    let err = b
        .add_call(add, target, vec![uint(5), Arg::Ref(x[0].clone())], U256::ZERO)
        .unwrap_err();
...
fn rejects_arg_count_mismatch() {
    ...
    let mut b = TransactionBuilder::new();
    // add expects 2 args
    let err = b.add_call(add, Address::ZERO, vec![uint(1)], U256::ZERO).unwrap_err();
...
fn rejects_dynamic_ref_without_length() {
    ...
    let mut b = TransactionBuilder::new();
    let s = b.add_call(get_string, Address::ZERO, vec![], U256::ZERO).unwrap();
    // pass the dynamic ref to setText WITHOUT calling with_length() first
    let err = b
        .add_call(set_text, Address::ZERO, vec![Arg::Ref(s[0].clone())], U256::ZERO)
        .unwrap_err();
...
fn rejects_more_than_three_vars() {
    ...
    let mut b = TransactionBuilder::new();
    let r = b.add_call(get_four, Address::ZERO, vec![], U256::ZERO).unwrap();
