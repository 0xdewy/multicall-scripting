...
//! encode→decode roundtrips, boundary validation, and unimplemented-flag rejection.
...
proptest! {
    ...
    fn static_partial_roundtrip((mt, rl, ro, size) in partial_inputs()) {
        ...
        let d = decode_partial_return(offset);
...
fn rejects_oversized_fields() {
    ...
    assert_eq!(static_call(over_120, U256::ZERO), Err(CodecError::MemTargetTooLarge));
    assert_eq!(static_call(U256::ZERO, over_120), Err(CodecError::ResultLengthTooLarge));
    assert_eq!(state_changing_call(256), Err(CodecError::MsgValueIndexTooLarge));
    assert_eq!(
...
#[test]
fn rejects_unimplemented_delegate_call() {
    // 0xFD (DELEGATE_CALL_FLAG) is declared but marked unimplemented in the schema.
    let offset = U256::from(DELEGATE_CALL_FLAG) << (VALUE_OFFSET as usize);
    match validate_offset(offset) {
        Err(CodecError::UnimplementedCalltype(name)) => assert_eq!(name, "DELEGATE_CALL_FLAG"),
        other => panic!("expected UnimplementedCalltype, got {other:?}"),
    }
    // Implemented flags validate cleanly.
